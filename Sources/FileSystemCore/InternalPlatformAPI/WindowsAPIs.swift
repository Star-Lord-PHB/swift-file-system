...
enum WindowsAPI {
    ...
    package static func pSidToString(sidPtr: borrowing UnsafeOwnedAutoResource) throws(SystemError) -> String {
        return try pSidToString(sidPtr: sidPtr.unownedView())
    }


    package static func stringToPsid(sidStr: String) throws(SystemError) -> UnsafeOwnedAutoResource {
    ...
    package static func windowsAcePermissionBits(fromPosixPermissionBits bits: CModeT, forDir: Bool = false) -> DWORD {
    ...
    package static func getCurrentProcessTokenHandle() throws(SystemError) -> UnsafeOwnedAutoResource {
    ...
    package static func getTokenInformation<T>(
    ...
    package static func allocateSid(
    ...
    package static func makeAcl(from aclEntries: UnsafeUnownedBufferPointer<EXPLICIT_ACCESSW>) throws(SystemError) -> UnsafeOwnedAutoPointer<ACL> {
        return try setEntriesInAcl(for: nil, entires: aclEntries)
    }


    package static func setEntriesInAcl(
    ...
    package static func makeSelfRelativeSecurityDescriptor(
    ...
    package static func securityDescriptor(
        fromPosixPermissions permissions: FilePermissions, 
        forDir: Bool = false
    ) throws(SystemError) -> UnsafeOwnedAutoPointer<SECURITY_DESCRIPTOR> {
    ...
    package static func getControl(from securityDescriptorPtr: UnsafeUnownedPointer<SECURITY_DESCRIPTOR>) throws(SystemError) -> (SECURITY_DESCRIPTOR_CONTROL, DWORD) {
    ...
    package static func getOwnerSid(from securityDescriptorPtr: UnsafeUnownedPointer<SECURITY_DESCRIPTOR>) throws(SystemError) -> (sid: UnsafeUnownedResource, defaulted: Bool) {
    ...
    @_lifetime(copy securityDescriptorPtr)
    package static func getGroupSid(from securityDescriptorPtr: UnsafeUnownedPointer<SECURITY_DESCRIPTOR>) throws(SystemError) -> (sid: UnsafeUnownedResource, defaulted: Bool) {
    ...
    package static func equalSid(sid1: UnsafeUnownedResource, sid2: UnsafeUnownedResource) -> Bool {
        return EqualSid(sid1.unsafeResourcePtr, sid2.unsafeResourcePtr)
    }

    
    package static func getSidLength(sidPtr: UnsafeUnownedResource) -> DWORD {
        return GetLengthSid(sidPtr.unsafeResourcePtr)
    }


    package static func createWellKnownSid(type: WELL_KNOWN_SID_TYPE, domainSid: UnsafeUnownedResource? = nil) throws(SystemError) -> UnsafeOwnedAutoResource {
