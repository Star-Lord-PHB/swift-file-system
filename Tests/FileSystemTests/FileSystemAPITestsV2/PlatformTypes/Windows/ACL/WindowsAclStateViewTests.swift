...
extension PlatformTypesAPITests.WindowsSecurityTests {
    ...
    @Suite("ACL state view")
    struct WindowsAclStateViewTests {}
...
//
// `WindowsRawAclStateView` is `~Escapable`, so a bare `#expect(state.isAbsent)` fails to compile:
...
extension PlatformTypesAPITests.WindowsSecurityTests.WindowsAclStateViewTests {
    ...
    private static func makeDescriptorWithNullDacl() -> WindowsSelfRelativeSecurityDescriptor {
        WindowsAbsoluteSecurityDescriptor(dacl: .null).makeSelfRelative()
    }
    ...
    @Test
    func `detach preserves the absent and null states`() {

        #expect(Self.makeDescriptorWithoutAcls().dacl.detach().case == .absent)
        #expect(Self.makeDescriptorWithNullDacl().dacl.detach().case == .null)
