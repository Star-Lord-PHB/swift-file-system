...
extension FileHandleAPITests.AppendTests.WindowsCreationSecurityTests {
    ...
    func `Truncate preserves existing security`() throws {
        ...
        let path = workspace.path("file")
        let initialHandle = try AppendHandle(
            forFileAt: path,
            creationPermissions: makeSampleSecurityDescriptor()
        )
        _ = try initialHandle.append(ByteBuffer("contents".utf8))
