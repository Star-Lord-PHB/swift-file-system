...
extension AsyncFileHandleAPITests.AppendTests.POSIXCreationPermissionTests {
    ...
    func `Custom creation permissions respect umask`() async throws {
        ...
        let handle = try await AsyncAppendHandle(forFileAt: path, creationPermissions: requested)
