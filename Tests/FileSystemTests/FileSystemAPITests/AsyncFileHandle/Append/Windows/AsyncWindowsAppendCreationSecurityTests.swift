...
extension AsyncFileHandleAPITests.AppendTests.WindowsCreationSecurityTests {
    ...
    func `Descriptor view applies creation security`() async throws {
        ...
        let executor = AsyncFileSystemExecutor(label: "append-sd", maximumThreadCount: 1)
    ...
    func `Absolute descriptor applies creation security`() async throws {
        ...
        let handle = try await AsyncAppendHandle(forFileAt: path, creationPermissions: security)
    ...
    func `Pre-cancelled descriptor open reports cancellation before creating`() async throws {
        ...
        await Support.expectPreCancelled {
            let security = Self.makeSampleSecurityDescriptor()
            _ = try await AsyncAppendHandle(forFileAt: path, creationPermissions: security.view)
        }
