...
extension AsyncFileHandleAPITests.WriteTests.POSIXCreationPermissionTests {
    ...
    func `Custom creation permissions respect umask`() async throws {
        ...
        let handle = try await AsyncWriteFileHandle(forFileAt: path, creationPermissions: requested)
